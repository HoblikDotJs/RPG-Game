...
function showPlayer() {
  ...
  <div class='container' style='margin-top:200px;'>
    <div class='row' id='invent'>
      </div>
    <div id='invBtns' style='margin-top: 510px;'class='row'>
      </div>
  </div>`);
  $("#screen").append(parent);
  $("#invent").append(
    $(`<div class='col-lg-6' id='stats' style='position:absolute; border:solid black; height:500px; width:500px;'>
         </div>`)
  );
  $("#invent").append(
    $(`<div class='col-lg-6' id='me' style='position: absolute; border:solid grey; height: 500px; width: 500px; margin-left: 550px;'>
         </div>`)
  );
...
function changeInvItem() {
  ...
  parent.append($("<center><b><p style='height:55px; font-size:25px;'>" + selectedItem.name + "</p></b></center>"));
  parent.append($("<center><p style='height:50px; font-size:20px'>" + "Slot : " + con(selectedItem.slot) + "</p></center>"));
  parent.append($("<center><p style='height:50px; font-size:20px;'>" + "Price : " + selectedItem.price + " gold </p></center>"));
  for (let property in selectedItem.properties) {
    parent.append(
      $('<center><p style="height:50px; font-size: 20px">' + con(property) + ": " + selectedItem.properties[property] + '</p></center>')
    );
  }
}

function forwardIBtn() {
  $("#invent").append($("<button class='btn btn-dark' style='margin-left:80px; margin-top:510px'> -> </button>")
    .click(() => {
      invSelected++;
...
function backwardIBtn() {
  $("#invent").append($("<button class='btn btn-dark' style='margin-top:510px'> <- </button>")
    .click(() => {
      invSelected--;
...
function putOnButton() {
  screenButtons.putOnBtn = $("<button class='btn btn-dark' style='margin-top:510px; margin-left:90px;'>Equip</button>")
    .appendTo("#invent");
